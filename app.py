...
def index():
    ...
    cur = conn.cursor()
    cur.execute('SELECT "code", "example" FROM "Countries"')
    ...
    cur.execute('SELECT "EmployerID", "Name" FROM "Employers"')
...
def register():
    ...
    if result:
        ...
        cur.execute('INSERT INTO "Employers" ("Name", "Country_ID") VALUES (%s, %s) RETURNING "EmployerID"', (employer, country))
...
def save_score():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('INSERT INTO "Highscores" ("Timestamp", "Name", "Score") VALUES (%s, %s, %s)', (datetime.now(), name, score))
    conn.commit()
    cur.close()
    conn.close()
...
def get_highscores():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('SELECT "Name", "Score" FROM "Highscores" ORDER BY "Score" DESC LIMIT 5')
